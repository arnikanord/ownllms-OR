...
export default function createClient(modelId: string) {
  ...
  const headers: Record<string, string> = {
    'HTTP-Referer': typeof window !== 'undefined' ? window.location.origin : process.env.SITE_URL || 'http://localhost:3001',
    'X-Title': typeof window !== 'undefined' ? (import.meta.env.VITE_SITE_NAME || 'Grog Chat') : (process.env.SITE_NAME || 'Grog Chat')
  };

  // Add OpenRouter specific header
  if (config.baseUrl.includes('openrouter.ai')) {
    headers['Authorization'] = `Bearer ${typeof window !== 'undefined' ? import.meta.env.VITE_OPENROUTER_API_KEY : process.env.OPENROUTER_API_KEY}`;
  }

  return {
    type: config.apiType,
    client: new OpenAI({
      apiKey: typeof window !== 'undefined' ? import.meta.env.VITE_OPENROUTER_API_KEY : process.env.OPENROUTER_API_KEY,
